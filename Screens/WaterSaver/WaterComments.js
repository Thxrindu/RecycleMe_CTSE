...
function WaterComments({ route }) {
    ...
    const UId = '1234';
    const cmntRef = firebase
    .firestore()
    .collection('waterSaverComments')
    // .doc(id) //need to check idea ID here

    const [isModalVisible, setIsModalVisible] = useState(false);
    const handleModal = () => setIsModalVisible(() => !isModalVisible);

    const getIdeaById = () => {
        cmntRef.onSnapshot(querySnapshot => {
            const comments = []
                            ...
                            {cmt.userId === UId ?
                                <View style={styles.fixToText}>
                                    ...
                                    <TouchableOpacity onPress={() => openModal(cmt._id,cmt.comment)}>
                                        <Image source={require('../../assets/WaterSaver/pensil.png')} />
                                    </TouchableOpacity>

                                    <TouchableOpacity onPress={() => deleteCmnt(cmt._id)}>
                                        <Image source={require('../../assets/WaterSaver/cross.png')} style={{ marginTop: 8 }} />
                                    </TouchableOpacity>
                                </View>
                                :
                                <View style={styles.fixToText}>
                                    ...
                                    <TouchableOpacity>
                                        <Image style={styles.icon} source={require('../../assets/WaterSaver/pensil.png')} />
                                    </TouchableOpacity>

                                    <TouchableOpacity>
                                        <Image style={styles.icon} source={require('../../assets/WaterSaver/cross.png')} />
                                    </TouchableOpacity>
                                </View>
                            }
